<div class="relative inline-block w-full"
    x-data="{
        ...
        viewMonth: {{ $viewMonth }},
        min: '{{ $min }}',
        hasRestriction: {{ $hasEnabledDatesRestriction ? 'true' : 'false' }},
        ...
        get calendarDays() {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (this.hasRestriction && !this.enabledDates.includes(dateStr)) {
                    disabled = true;
                }
        ...
        selectDate(dayObj) {
            ...
            $wire.selectDateString(dayObj.dateStr);
            this.isOpen = false;
        }
    }"
...
>
    ...
    <div class="mt-2 relative">
        ...
        @else
            <button type="button" @click="isOpen = !isOpen" class="flex h-12 w-full items-center justify-between rounded-xl border border-slate-300 bg-white px-4 py-3 text-left text-slate-900 shadow-sm transition hover:border-[#216417] focus:outline-none focus:ring-2 focus:ring-[#216417]/20">
                <span class="truncate">{{ $value ? date('M d, Y', strtotime($value)) : $placeholder }}</span>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.045l3.71-3.815a.75.75 0 111.08 1.04l-4.25 4.375a.75.75 0 01-1.08 0L5.21 8.27a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
                </svg>
        ...
        <div x-show="isOpen" x-cloak class="absolute left-0 right-0 mt-2 rounded-xl border border-slate-200 bg-white p-4 shadow-xl z-50">
                        ...
                        <template x-if="day !== null">
                            <button
                                ...
                                :class="{
                                    ...
                                    'bg-[#216417] text-white shadow-md': day.dateStr === '{{ $value }}',
                                    'bg-slate-50 text-slate-300 cursor-not-allowed line-through': day.disabled && day.dateStr !== '{{ $value }}',
                                    'bg-white text-slate-700 hover:bg-slate-100 hover:text-slate-900': !day.disabled && day.dateStr !== '{{ $value }}'
                                }"
                                x-text="day.day"
                            ></button>
                        </template>
