<<<<<<< HEAD
# Graph Report - Amiga-Travel  (2026-07-25)

## Corpus Check
- 436 files · ~412,355 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5541 nodes · 13155 edges · 316 communities (282 shown, 34 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 1230 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `224ac579`
=======
# Graph Report - amiga-gracia  (2026-07-25)

## Corpus Check
- 433 files · ~410,206 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5495 nodes · 13174 edges · 312 communities (277 shown, 35 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 1272 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1dafff4d`
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- HTTP Controllers & Routing (C0)
- Data Models & Domain (C1)
- HTTP Controllers & Routing (C2)
- Core Module 3
- Core Module 4
- Core Module 5
- Core Module 6
- Data Models & Domain (C7)
- Core Module 8
- Database Schema (C9)
- Core Module 10
- Database Schema (C11)
- Data Models & Domain (C12)
- Core Module 13
- Core Module 14
- Core Module 15
- Data Models & Domain (C16)
- Core Module 17
- HTTP Controllers & Routing (C18)
- HTTP Controllers & Routing (C19)
- Core Module 20
- Data Models & Domain (C21)
- Core Module 22
- Core Module 23
- Filament Admin & UI (C24)
- Data Models & Domain (C25)
- Data Models & Domain (C26)
- Core Module 27
- Core Module 28
- Database Schema (C29)
- Core Module 30
- Frontend & Components (C31)
- Core Module 32
- Core Module 33
- Data Models & Domain (C34)
- Data Models & Domain (C35)
- Core Module 36
- Filament Admin & UI (C37)
- Core Module 38
- Data Models & Domain (C39)
- HTTP Controllers & Routing (C40)
- Data Models & Domain (C41)
- Filament Admin & UI (C42)
- Core Module 43
- Core Module 44
- Core Module 45
- HTTP Controllers & Routing (C46)
- Core Module 47
- Data Models & Domain (C48)
- Data Models & Domain (C49)
- Data Models & Domain (C50)
- Core Module 51
- Data Models & Domain (C52)
- Filament Admin & UI (C53)
- Core Module 54
- HTTP Controllers & Routing (C55)
- Core Module 56
- Data Models & Domain (C57)
- Core Module 58
- Core Module 59
- Filament Admin & UI (C60)
- Data Models & Domain (C61)
- Core Module 62
- Data Models & Domain (C63)
- Data Models & Domain (C64)
- Core Module 65
- HTTP Controllers & Routing (C66)
- Core Module 67
- Database Schema (C68)
- Core Module 69
- Database Seeders & Testing (C70)
- Data Models & Domain (C71)
- Core Module 72
- HTTP Controllers & Routing (C73)
- Data Models & Domain (C74)
- Core Module 75
- Core Module 76
- Core Module 77
- Core Module 78
- Core Module 79
- HTTP Controllers & Routing (C80)
- Core Module 81
- Core Module 82
- Core Module 83
- Core Module 84
- Core Module 85
- Core Module 87
- Core Module 88
- Core Module 89
- Filament Admin & UI (C90)
- Core Module 91
- Core Module 92
- Core Module 93
- Data Models & Domain (C94)
- Filament Admin & UI (C95)
- Filament Admin & UI (C96)
- Core Module 97
- Core Module 98
- Filament Admin & UI (C100)
- Core Module 101
- Core Module 102
- Resource
- Filament Admin & UI (C104)
- Filament Admin & UI (C105)
- Filament Admin & UI (C106)
- Filament Admin & UI (C107)
- Core Module 108
- Core Module 109
- Core Module 110
- Core Module 111
- Core Module 112
- User
- Core Module 114
- Core Module 115
- qt
<<<<<<< HEAD
- Database Seeders & Testing (C118)
=======
- _each
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288
- Core Module 119
- Vehicle
- Core Module 121
- Core Module 122
- Core Module 123
- Core Module 124
- Core Module 125
- Core Module 126
- Core Module 127
- Core Module 128
- Core Module 129
- Core Module 130
- Core Module 131
- Core Module 132
- Core Module 133
- Core Module 134
- Core Module 135
- Core Module 136
- Core Module 137
- Core Module 138
- Core Module 139
- Core Module 205
- Core Module 206
- Core Module 207
- Core Module 208
- Core Module 209
- Core Module 210
- Core Module 211
- Core Module 212
- Core Module 213
- Core Module 214
- Core Module 215
- Core Module 216
- Core Module 217
- Core Module 222
- Core Module 223
- ScheduleResource.php
- Core Module 225
- Core Module 227
- ManageProofs
- graphify reference: add a URL and watch a folder
- Filament\Resources\Pages\ViewRecord
- getDataset
- .nextStep
- qt
<<<<<<< HEAD
- Collection
- Illuminate\Database\Eloquent\Relations\HasOne
- Illuminate\Support\Facades\Validator

## God Nodes (most connected - your core abstractions)
1. `components/chart.js` - 948 edges
2. `stat/chart.js` - 612 edges
3. `_update()` - 88 edges
4. `x()` - 85 edges
5. `BookingForm` - 84 edges
6. `_update()` - 84 edges
7. `Booking` - 76 edges
8. `te()` - 74 edges
9. `V()` - 72 edges
10. `draw()` - 55 edges
=======
- yn
- Illuminate\Support\Facades\Validator

## God Nodes (most connected - your core abstractions)
1. `_update()` - 88 edges
2. `x()` - 85 edges
3. `BookingForm` - 84 edges
4. `_update()` - 84 edges
5. `Booking` - 82 edges
6. `te()` - 74 edges
7. `V()` - 72 edges
8. `Schedule` - 59 edges
9. `draw()` - 55 edges
10. `vd()` - 53 edges
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

## Surprising Connections (you probably didn't know these)
- `te()` --indirect_call--> `Pr()`  [INFERRED]
  public/js/filament/forms/components/markdown-editor.js → public/js/filament/filament/echo.js
- `getExtension()` --indirect_call--> `Ht()`  [INFERRED]
  public/js/filament/forms/components/file-upload.js → public/js/filament/forms/components/markdown-editor.js
- `_getTestState()` --indirect_call--> `Ht()`  [INFERRED]
  public/js/filament/forms/components/file-upload.js → public/js/filament/forms/components/markdown-editor.js
- `wWinMain()` --calls--> `CreateAndAttachConsole()`  [INFERRED]
  flutter_app/windows/runner/main.cpp → flutter_app/windows/runner/utils.cpp
- `Win32Window::Win32Window()` --calls--> `Destroy`  [INFERRED]
  flutter_app/windows/runner/win32_window.cpp → flutter_app/windows/runner/win32_window.h

## Import Cycles
- None detected.

<<<<<<< HEAD
## Communities (316 total, 34 thin omitted)

### Community 0 - "HTTP Controllers & Routing (C0)"
Cohesion: 0.01
Nodes (324): bool get, Color, dart:async, dart:convert, DateTime, double?, _accommodations, _activePassengerIndex (+316 more)

### Community 1 - "Data Models & Domain (C1)"
Cohesion: 0.01
Nodes (113): components/chart.js, acquireContext(), addControllers(), addPlugins(), addScales(), afterDraw(), alpha(), Au() (+105 more)

### Community 2 - "HTTP Controllers & Routing (C2)"
Cohesion: 0.02
Nodes (118): activateAttributeIfSupported(), appendStringToTextAtIndex(), applyBlockAttribute(), attachmentDidChangeUploadProgress(), attachmentIsManaged(), Ca(), canRedo(), canSyncDocumentView() (+110 more)

### Community 3 - "Core Module 3"
Cohesion: 0.02
Nodes (19): AccommodationResource, ApkUserResource, AppNotificationResource, BookingResource, DiscountResource, GraciaEarningRuleResource, InquiryResource, ScheduleResource (+11 more)

### Community 4 - "Core Module 4"
Cohesion: 0.04
Nodes (164): _a(), Ac(), Ae(), af(), ai(), al(), An(), ao() (+156 more)

### Community 5 - "Core Module 5"
Cohesion: 0.02
Nodes (131): disabled(), So(), stat/chart.js, aa(), alpha(), an(), aspectRatio(), At() (+123 more)

### Community 6 - "Core Module 6"
Cohesion: 0.03
Nodes (111): _a(), abutsStart(), after(), afterAutoSkip(), Ag(), Ai(), before(), buildLookupTable() (+103 more)

### Community 7 - "Data Models & Domain (C7)"
Cohesion: 0.05
Nodes (33): AdminNotifications, ManagePaymentSettings, ApkUserResource/RelationManagers/BookingsRelationManager.php, BookingsRelationManager, GraciaPointLedgersRelationManager, AccommodationsRelationManager, PassengersRelationManager, BookingResource/RelationManagers/TransportClassesRelationManager.php (+25 more)

### Community 8 - "Core Module 8"
Cohesion: 0.03
Nodes (127): addAttribute(), addAttributeAtRange(), addAttributesAtRange(), addHTMLAttribute(), appendText(), applyBlockAttributeAtRange(), breakFormattedBlock(), breaksOnReturn() (+119 more)

### Community 9 - "Database Schema (C9)"
Cohesion: 0.03
Nodes (13): PromotionalTicketResource, FerryRoute, PromotionalTicket, Schedule, TransportClass, Vehicle, FerryRouteSeeder, TransportClassSeeder (+5 more)

### Community 10 - "Core Module 10"
Cohesion: 0.09
Nodes (65): [g](), [x](), $c(), D(), E(), Ea(), g(), H() (+57 more)

### Community 11 - "Database Schema (C11)"
Cohesion: 0.05
Nodes (48): ArrowLeft(), ArrowRight(), attachmentManagerDidRequestRemovalOfAttachment(), backspace(), compositionControllerDidRequestRemovalOfAttachment(), d(), delete(), deleteByComposition() (+40 more)

### Community 12 - "Data Models & Domain (C12)"
Cohesion: 0.05
Nodes (90): adjustHitBoxes(), ae(), af(), calculateLabelRotation(), cd(), _computeGridLineItems(), _computeLabelArea(), _computeTitleHeight() (+82 more)

### Community 13 - "Core Module 13"
Cohesion: 0.05
Nodes (12): Booking, Inquiry, Transaction, Voucher, ReportingService, BelongsTo, BelongsToMany, HasMany (+4 more)

### Community 14 - "Core Module 14"
Cohesion: 0.05
Nodes (65): aa(), ad(), Ah(), applyStack(), bf(), buildTicks(), _calculateBarIndexPixels(), _calculateBarValuePixels() (+57 more)

### Community 15 - "Core Module 15"
Cohesion: 0.11
Nodes (13): AdminNotificationController, AuthController, AdminMiddleware, EnsureStaffPermission, AdminPanelProvider, Closure, Filament\Panel, Filament\PanelProvider (+5 more)

### Community 16 - "Data Models & Domain (C16)"
Cohesion: 0.06
Nodes (43): ba(), bi(), c(), ca(), clickPercent(), constructor(), e(), getExtension() (+35 more)

### Community 17 - "Core Module 17"
Cohesion: 0.11
Nodes (76): Sg(), ad(), at(), B(), br(), Bt(), ca(), cd() (+68 more)

### Community 18 - "HTTP Controllers & Routing (C18)"
Cohesion: 0.07
Nodes (3): BookingForm, Collection, WithFileUploads

### Community 19 - "HTTP Controllers & Routing (C19)"
Cohesion: 0.03
Nodes (97): ht(), ar(), Bi(), Bl(), Ce(), cf(), chartOptionScopes(), clone() (+89 more)

### Community 20 - "Core Module 20"
Cohesion: 0.04
Nodes (95): addBox(), addEventListener(), afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterFit(), afterSetDimensions(), afterTickToLabelConversion() (+87 more)
=======
## Communities (312 total, 35 thin omitted)

### Community 0 - "HTTP Controllers & Routing (C0)"
Cohesion: 0.01
Nodes (315): bool get, Color, dart:async, dart:convert, DateTime, double?, _accommodations, _activePassengerIndex (+307 more)

### Community 1 - "Data Models & Domain (C1)"
Cohesion: 0.01
Nodes (100): _a(), acquireContext(), addControllers(), addPlugins(), addScales(), afterAutoSkip(), Ai(), beforeDatasetDraw() (+92 more)

### Community 2 - "HTTP Controllers & Routing (C2)"
Cohesion: 0.02
Nodes (116): activateAttributeIfSupported(), appendStringToTextAtIndex(), applyBlockAttribute(), attachmentDidChangeUploadProgress(), attachmentIsManaged(), attributeChangedCallback(), Ca(), canRedo() (+108 more)

### Community 3 - "Core Module 3"
Cohesion: 0.02
Nodes (19): AccommodationResource, ApkUserResource, AppNotificationResource, BookingResource, DiscountResource, FerryRouteResource, GraciaEarningRuleResource, InquiryResource (+11 more)

### Community 4 - "Core Module 4"
Cohesion: 0.04
Nodes (162): u(), be(), _a(), Ac(), ad(), Ae(), ai(), al() (+154 more)

### Community 5 - "Core Module 5"
Cohesion: 0.02
Nodes (119): Yn(), aa(), alpha(), an(), aspectRatio(), be(), beforeDatasetDraw(), beforeDatasetsDraw() (+111 more)

### Community 6 - "Core Module 6"
Cohesion: 0.03
Nodes (108): abutsStart(), after(), Al(), ar(), as(), before(), Bi(), count() (+100 more)

### Community 7 - "Data Models & Domain (C7)"
Cohesion: 0.06
Nodes (21): AdminNotifications, ManagePaymentSettings, BookingsRelationManager, GraciaPointLedgersRelationManager, AccommodationsRelationManager, PassengersRelationManager, TransportClassesRelationManager, ScheduleAccommodationsRelationManager (+13 more)

### Community 8 - "Core Module 8"
Cohesion: 0.03
Nodes (119): addAttribute(), addAttributeAtRange(), addAttributesAtRange(), addHTMLAttribute(), appendText(), applyBlockAttributeAtRange(), breakFormattedBlock(), canBeGroupedWith() (+111 more)

### Community 9 - "Database Schema (C9)"
Cohesion: 0.03
Nodes (15): CreateFerryRoute, EditFerryRoute, PromotionalTicketResource, ListVehicles, ScheduleController, FerryRoute, PromotionalTicket, Schedule (+7 more)

### Community 10 - "Core Module 10"
Cohesion: 0.08
Nodes (64): [g](), [x](), $c(), ca(), D(), E(), g(), H() (+56 more)

### Community 11 - "Database Schema (C11)"
Cohesion: 0.04
Nodes (108): attachFiles(), backspace(), breaksOnReturn(), canDecreaseBlockAttributeLevel(), canSetCurrentAttribute(), canSetCurrentBlockAttribute(), createLinkHTML(), cut() (+100 more)

### Community 12 - "Data Models & Domain (C12)"
Cohesion: 0.04
Nodes (116): ad(), adjustHitBoxes(), ae(), af(), afterDraw(), aspectRatio(), bf(), bn() (+108 more)

### Community 13 - "Core Module 13"
Cohesion: 0.04
Nodes (11): BookingController, Booking, Inquiry, Passenger, Transaction, UserLoginHistory, VoucherRedemption, ReportingService (+3 more)

### Community 14 - "Core Module 14"
Cohesion: 0.05
Nodes (56): aa(), Ah(), applyStack(), _calculateBarIndexPixels(), _calculateBarValuePixels(), calculateCircumference(), _circumference(), countVisibleElements() (+48 more)

### Community 15 - "Core Module 15"
Cohesion: 0.05
Nodes (24): CreateUser, EditUser, AdminNotificationController, AuthController, AdminMiddleware, EnsureStaffPermission, AdminNotificationStatus, HasOne (+16 more)

### Community 16 - "Data Models & Domain (C16)"
Cohesion: 0.06
Nodes (50): ba(), bi(), c(), ca(), clickPercent(), constructor(), de(), define() (+42 more)

### Community 17 - "Core Module 17"
Cohesion: 0.10
Nodes (80): Sg(), at(), B(), br(), Bt(), cd(), Cr(), Ct() (+72 more)

### Community 19 - "HTTP Controllers & Routing (C19)"
Cohesion: 0.03
Nodes (91): Bl(), Ce(), cf(), chartOptionScopes(), clone(), constructor(), create(), describe() (+83 more)

### Community 20 - "Core Module 20"
Cohesion: 0.04
Nodes (79): addBox(), afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterFit(), afterSetDimensions(), afterTickToLabelConversion(), afterUpdate() (+71 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 21 - "Data Models & Domain (C21)"
Cohesion: 0.04
Nodes (13): Bi(), bn(), Id(), ji(), kd(), qi(), Ri(), te() (+5 more)

### Community 22 - "Core Module 22"
<<<<<<< HEAD
Cohesion: 0.06
Nodes (72): attachFiles(), canSetCurrentAttribute(), compositionShouldAcceptFile(), createLinkHTML(), decreaseListLevel(), deleteByDrag(), drop(), findRangesOfBlocks() (+64 more)
=======
Cohesion: 0.08
Nodes (37): Ao(), applyStack(), as(), _calculateBarIndexPixels(), _calculateBarValuePixels(), calculateCircumference(), _circumference(), _computeAngle() (+29 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 23 - "Core Module 23"
Cohesion: 0.06
Nodes (39): apply(), ar(), B(), co(), Cr(), es(), Et(), fo() (+31 more)

### Community 24 - "Filament Admin & UI (C24)"
<<<<<<< HEAD
Cohesion: 0.08
Nodes (32): Ac(), an(), color(), darken(), Dc(), desaturate(), eo(), fo() (+24 more)

### Community 25 - "Data Models & Domain (C25)"
Cohesion: 0.13
Nodes (23): e(), i(), l(), Ni(), o(), t(), u(), be() (+15 more)

### Community 26 - "Data Models & Domain (C26)"
Cohesion: 0.09
Nodes (29): average(), dataset(), _eventHandler(), fn(), getCenterPoint(), getProps(), hasValue(), hn() (+21 more)

### Community 27 - "Core Module 27"
Cohesion: 0.07
Nodes (57): _a(), add(), ba(), buildOrUpdateScales(), C(), cl(), Co(), cr() (+49 more)
=======
Cohesion: 0.06
Nodes (41): Ac(), an(), Au(), ba(), bu(), color(), darken(), Dc() (+33 more)

### Community 25 - "Data Models & Domain (C25)"
Cohesion: 0.29
Nodes (6): e(), i(), l(), Ni(), o(), t()

### Community 26 - "Data Models & Domain (C26)"
Cohesion: 0.09
Nodes (31): At(), average(), dataset(), _eventHandler(), Fa(), getBasePixel(), getBasePosition(), getBaseValue() (+23 more)

### Community 27 - "Core Module 27"
Cohesion: 0.12
Nodes (29): C(), Co(), cr(), endOf(), Et(), format(), formats(), getLabelAndValue() (+21 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 28 - "Core Module 28"
Cohesion: 0.05
Nodes (18): ListAccommodations, ListApkUsers, ListAppNotifications, ListBookings, ListDiscounts, ListFerryRoutes, ListGraciaEarningRules, ListInquiries (+10 more)

### Community 29 - "Database Schema (C29)"
Cohesion: 0.06
Nodes (24): actions(), button(), constructor(), danger(), dispatch(), dispatchSelf(), dispatchTo(), duration() (+16 more)

### Community 30 - "Core Module 30"
<<<<<<< HEAD
Cohesion: 0.06
Nodes (19): GraciaPointLedger, Passenger, ScheduleAccommodation, TourDate, VehicleModel, VehicleRate, VoucherRedemption, DatabaseSeeder (+11 more)
=======
Cohesion: 0.09
Nodes (33): buildOrUpdateScales(), cl(), _computeLabelSizes(), Ct(), D(), data(), E(), ensureScalesHaveIDs() (+25 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 31 - "Frontend & Components (C31)"
Cohesion: 0.07
Nodes (44): ActivityScreen, _ActivityScreenState, BookingDetailsScreen, _BookingDetailsScreenState, BookingSubmitScreen, _BookingSubmitScreenState, ContactScreen, _ContactScreenState (+36 more)

### Community 32 - "Core Module 32"
<<<<<<< HEAD
Cohesion: 0.05
Nodes (65): afterAutoSkip(), Ao(), applyStack(), as(), Bi(), buildLookupTable(), _calculateBarIndexPixels(), _calculateBarValuePixels() (+57 more)

### Community 33 - "Core Module 33"
Cohesion: 0.09
Nodes (41): adjustHitBoxes(), afterDraw(), bc(), Bl(), clear(), _computeLabelArea(), _computeTitleHeight(), _createItems() (+33 more)

### Community 34 - "Data Models & Domain (C34)"
Cohesion: 0.10
Nodes (5): OverallReports, Collection, StaffPerformance, DatePicker, Component

### Community 35 - "Data Models & Domain (C35)"
Cohesion: 0.08
Nodes (32): average(), ch(), cu(), dataset(), en(), eu(), getCenterPoint(), getMaximumSize() (+24 more)

### Community 36 - "Core Module 36"
Cohesion: 0.05
Nodes (72): addElements(), as(), aspectRatio(), At(), Bs(), buildOrUpdateElements(), C(), Ca() (+64 more)
=======
Cohesion: 0.11
Nodes (23): ac(), Ai(), Bi(), ca(), determineDataLimits(), getMinMax(), _getOtherScale(), getUserBounds() (+15 more)

### Community 33 - "Core Module 33"
Cohesion: 0.06
Nodes (64): $h(), acquireContext(), adjustHitBoxes(), bc(), Bl(), clear(), _computeLabelArea(), _computeTitleHeight() (+56 more)

### Community 34 - "Data Models & Domain (C34)"
Cohesion: 0.07
Nodes (8): OverallReports, Collection, StaffPerformance, DatePicker, PaymentProof, UserDashboard, Livewire\Component, Livewire\WithFileUploads

### Community 35 - "Data Models & Domain (C35)"
Cohesion: 0.09
Nodes (12): BookingExportController, DatabaseSeeder, DiscountSeeder, ScheduleAccommodationSeeder, TourHotelsSeeder, TransportClassSeeder, VehicleRateSeeder, VehicleSeeder (+4 more)

### Community 36 - "Core Module 36"
Cohesion: 0.07
Nodes (37): average(), Ca(), cd(), clear(), cn(), Da(), Fc(), Fd() (+29 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 37 - "Filament Admin & UI (C37)"
Cohesion: 0.10
Nodes (29): afterDatasetsUpdate(), buildOrUpdateControllers(), _d(), _destroyDatasetMeta(), generateLabels(), getDatasetMeta(), getDataVisibility(), getMaxBorderWidth() (+21 more)

### Community 38 - "Core Module 38"
Cohesion: 0.06
Nodes (15): EditAccommodation, EditAppNotification, EditBooking, EditDiscount, EditFerryRoute, EditGraciaEarningRule, EditPromotionalTicket, EditSchedule (+7 more)

### Community 39 - "Data Models & Domain (C39)"
<<<<<<< HEAD
Cohesion: 0.07
Nodes (35): cacheViewForObject(), compositionDidLoadSnapshot(), createAttachmentNodes(), createChildView(), createContainerElement(), createDocumentFragmentForSync(), createElement(), createNodes() (+27 more)

### Community 40 - "HTTP Controllers & Routing (C40)"
Cohesion: 0.07
Nodes (49): oi(), add(), applyKeyboardCommand(), attachmentDidChangeAttributes(), attachmentEditorDidRequestRemovalOfAttachment(), attributeChangedCallback(), canBeGrouped(), checkValidity() (+41 more)

### Community 41 - "Data Models & Domain (C41)"
Cohesion: 0.06
Nodes (45): canAcceptDataTransfer(), canDecreaseNestingLevel(), canIncreaseNestingLevel(), compositionControllerDidFocus(), compositionDidRequestChangingSelectionToLocationRange(), createDOMRangeFromLocationRange(), createDOMRangeFromPoint(), createLocationRangeFromDOMRange() (+37 more)

### Community 42 - "Filament Admin & UI (C42)"
Cohesion: 0.09
Nodes (37): afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterFit(), afterSetDimensions(), afterTickToLabelConversion(), afterUpdate(), beforeBuildTicks() (+29 more)

### Community 43 - "Core Module 43"
Cohesion: 0.06
Nodes (54): acquireContext(), ar(), buildTicks(), calculateCircumference(), calculateLabelRotation(), _calculatePadding(), _circumference(), _computeAngle() (+46 more)

### Community 44 - "Core Module 44"
Cohesion: 0.06
Nodes (17): ListTours, AccommodationController, DiscountController, GraciaPointsController, PromotionController, Api/TourController.php, TourController, BookingExportController (+9 more)

### Community 45 - "Core Module 45"
Cohesion: 0.05
Nodes (24): DashboardStatsOverview, BookingCancellation, self, BookingConfirmation, BookingCreated, PaymentProofReceived, RebookingRequested, RebookingVerification (+16 more)

### Community 46 - "HTTP Controllers & Routing (C46)"
Cohesion: 0.11
Nodes (27): afterDatasetsUpdate(), generateLabels(), getDatasetMeta(), getDataVisibility(), _getLegendItemAt(), getMaxBorderWidth(), getStyle(), _handleEvent() (+19 more)

### Community 47 - "Core Module 47"
Cohesion: 0.06
Nodes (45): beforeinput(), canApplyToDocument(), compositionend(), compositionstart(), compositionupdate(), constructor(), elementDidMutate(), end() (+37 more)
=======
Cohesion: 0.05
Nodes (47): cacheViewForObject(), canSyncDocumentView(), compositionDidChangeDocument(), compositionDidLoadSnapshot(), createAttachmentNodes(), createChildView(), createContainerElement(), createDocumentFragmentForSync() (+39 more)

### Community 40 - "HTTP Controllers & Routing (C40)"
Cohesion: 0.07
Nodes (52): It(), add(), applyKeyboardCommand(), attachmentDidChangeAttributes(), attachmentEditorDidRequestRemovalOfAttachment(), box(), canBeGrouped(), checkValidity() (+44 more)

### Community 41 - "Data Models & Domain (C41)"
Cohesion: 0.07
Nodes (29): canAcceptDataTransfer(), compositionControllerDidFocus(), compositionDidRequestChangingSelectionToLocationRange(), createDOMRangeFromLocationRange(), createDOMRangeFromPoint(), createLocationRangeFromDOMRange(), didMouseDown(), domRangeWithinElement() (+21 more)

### Community 42 - "Filament Admin & UI (C42)"
Cohesion: 0.06
Nodes (51): afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterDraw(), afterFit(), afterSetDimensions(), afterTickToLabelConversion(), afterUpdate() (+43 more)

### Community 43 - "Core Module 43"
Cohesion: 0.06
Nodes (55): disabled(), afterAutoSkip(), ar(), buildLookupTable(), buildTicks(), calculateLabelRotation(), _calculatePadding(), _computeGridLineItems() (+47 more)

### Community 44 - "Core Module 44"
Cohesion: 0.05
Nodes (14): ListTours, DiscountController, PromotionController, TourController, VoucherController, Controller, TourController, Discount (+6 more)

### Community 45 - "Core Module 45"
Cohesion: 0.05
Nodes (32): AccommodationController, BookingCancellation, self, BookingConfirmation, BookingCreated, PaymentProofReceived, RebookingRequested, RebookingVerification (+24 more)

### Community 46 - "HTTP Controllers & Routing (C46)"
Cohesion: 0.11
Nodes (25): afterDatasetsUpdate(), generateLabels(), getDatasetMeta(), getDataVisibility(), _getLegendItemAt(), getMaxBorderWidth(), getStyle(), _handleEvent() (+17 more)

### Community 47 - "Core Module 47"
Cohesion: 0.07
Nodes (34): beforeinput(), canApplyToDocument(), compositionend(), compositionstart(), compositionupdate(), dragend(), elementDidMutate(), end() (+26 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 48 - "Data Models & Domain (C48)"
Cohesion: 0.25
Nodes (17): Ae(), at(), de(), dt(), fr(), Gt(), It(), jt() (+9 more)

### Community 49 - "Data Models & Domain (C49)"
Cohesion: 0.09
<<<<<<< HEAD
Nodes (28): Bt(), xo(), addEventListener(), bindEvents(), bindResponsiveEvents(), bindUserEvents(), _checkEventBindings(), cs() (+20 more)
=======
Nodes (27): Bt(), xo(), addEventListener(), bindEvents(), bindResponsiveEvents(), bindUserEvents(), cs(), di() (+19 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 50 - "Data Models & Domain (C50)"
Cohesion: 0.07
Nodes (5): ManageWebsiteSettings, WebsiteSetting, AppServiceProvider, Illuminate\Support\Facades\View, Illuminate\Support\ServiceProvider

### Community 51 - "Core Module 51"
<<<<<<< HEAD
Cohesion: 0.09
Nodes (11): CreateUser, EditUser, AdminNotificationStatus, HasOne, User, AdminNotificationFeed, Filament\Models\Contracts\FilamentUser, Illuminate\Database\Eloquent\SoftDeletes (+3 more)

### Community 52 - "Data Models & Domain (C52)"
Cohesion: 0.10
Nodes (26): add(), clear(), cn(), Da(), eh(), _getAnims(), _getLegendItemAt(), Gi() (+18 more)

### Community 53 - "Filament Admin & UI (C53)"
Cohesion: 0.06
Nodes (12): Action, DeleteAllUsers, PurgeExpiredProofs, ManageProofs, TransactionResource, PaymentSetting, self, Filament\Actions\Action (+4 more)
=======
Cohesion: 0.07
Nodes (32): Ag(), Ef(), fe(), features(), fromFormat(), fromHTTP(), fromRFC2822(), fromSQL() (+24 more)

### Community 52 - "Data Models & Domain (C52)"
Cohesion: 0.08
Nodes (31): addEventListener(), bindEvents(), bindResponsiveEvents(), bindUserEvents(), ch(), cu(), dataset(), dn() (+23 more)

### Community 53 - "Filament Admin & UI (C53)"
Cohesion: 0.10
Nodes (10): Action, DeleteAllUsers, PurgeExpiredProofs, ManageProofs, PaymentSetting, self, Filament\Actions\Action, Filament\Actions\Concerns\InteractsWithActions (+2 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 54 - "Core Module 54"
Cohesion: 0.10
Nodes (13): a(), ar(), at(), cr(), d(), f(), H(), ji() (+5 more)

### Community 55 - "HTTP Controllers & Routing (C55)"
<<<<<<< HEAD
Cohesion: 0.06
Nodes (48): Yn(), Ge(), active(), al(), _animateOptions(), _cachedScopes(), cancel(), chartOptionScopes() (+40 more)

### Community 56 - "Core Module 56"
Cohesion: 0.15
Nodes (4): FerryRouteResource, Form, Resource, Table

### Community 57 - "Data Models & Domain (C57)"
Cohesion: 0.09
Nodes (16): CreateAccommodation, CreateAppNotification, CreateBooking, CreateDiscount, CreateFerryRoute, CreateGraciaEarningRule, CreateInquiry, CreatePromotionalTicket (+8 more)
=======
Cohesion: 0.08
Nodes (38): _a(), active(), add(), _animateOptions(), ba(), _cachedScopes(), configure(), _createAnimations() (+30 more)

### Community 56 - "Core Module 56"
Cohesion: 0.09
Nodes (27): af(), bo(), En(), es(), Fr(), gd(), Ha(), Hc() (+19 more)

### Community 57 - "Data Models & Domain (C57)"
Cohesion: 0.10
Nodes (15): CreateAccommodation, CreateAppNotification, CreateBooking, CreateDiscount, CreateGraciaEarningRule, CreateInquiry, CreatePromotionalTicket, CreateSchedule (+7 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 58 - "Core Module 58"
Cohesion: 0.14
Nodes (37): _a(), aa(), ba(), Be(), Bi(), br(), Ca(), ce() (+29 more)

### Community 59 - "Core Module 59"
Cohesion: 0.11
Nodes (14): Any, Flutter, AppDelegate, Bool, RunnerTests, AppDelegate, Bool, RunnerTests (+6 more)

### Community 60 - "Filament Admin & UI (C60)"
<<<<<<< HEAD
Cohesion: 0.10
Nodes (21): box(), canSetCurrentTextAttribute(), cut(), didClickAttachment(), dragstart(), findAttachmentForElement(), fromUCS2String(), getAttachmentAndPositionById() (+13 more)
=======
Cohesion: 0.12
Nodes (8): GraciaPointsController, AppNotification, GraciaEarningRule, GraciaPointLedger, GraciaUserBalance, GraciaPointsService, GraciaEarningRuleSeeder, Illuminate\Database\Eloquent\Factories\HasFactory
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 61 - "Data Models & Domain (C61)"
Cohesion: 0.07
Nodes (26): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+18 more)

### Community 62 - "Core Module 62"
<<<<<<< HEAD
Cohesion: 0.10
Nodes (22): attachmentForFile(), attributesForFile(), didChangeAttributes(), getContentType(), getHeight(), getHref(), getPreviewURL(), getType() (+14 more)

### Community 63 - "Data Models & Domain (C63)"
Cohesion: 0.29
Nodes (5): VoucherService, PromotionalTicket, Schedule, Voucher, VoucherRedemption

### Community 64 - "Data Models & Domain (C64)"
Cohesion: 0.12
Nodes (23): ac(), Ai(), ca(), ec(), Fc(), G(), getIndexAngle(), getPointPosition() (+15 more)
=======
Cohesion: 0.07
Nodes (29): attachmentForFile(), attributesForFile(), canSetCurrentTextAttribute(), compositionControllerDidRequestDeselectingAttachment(), compositionDidStartEditingAttachment(), didClickAttachment(), dragstart(), findAttachmentForElement() (+21 more)

### Community 63 - "Data Models & Domain (C63)"
Cohesion: 0.12
Nodes (21): addControllers(), addPlugins(), addScales(), al(), cancel(), _createDescriptors(), _descriptors(), _each() (+13 more)

### Community 64 - "Data Models & Domain (C64)"
Cohesion: 0.15
Nodes (17): add(), eh(), _getAnims(), Gi(), gn(), has(), Jc(), listen() (+9 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 65 - "Core Module 65"
Cohesion: 0.33
Nodes (18): ai(), d(), Di(), f(), Ge(), ir(), ja(), ka() (+10 more)

### Community 66 - "HTTP Controllers & Routing (C66)"
Cohesion: 0.20
Nodes (14): active(), _animateOptions(), cancel(), _createAnimations(), _createDescriptors(), _descriptors(), kh(), _notify() (+6 more)

### Community 67 - "Core Module 67"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 68 - "Database Schema (C68)"
Cohesion: 0.10
Nodes (20): FlPluginRegistry, fl_register_plugins(), main(), my_application_activate(), my_application_class_init(), my_application_dispose(), my_application_init(), my_application_local_command_line() (+12 more)

### Community 69 - "Core Module 69"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 70 - "Database Seeders & Testing (C70)"
Cohesion: 0.15
Nodes (29): Cn(), da(), En(), fa(), Fi(), fn(), Ii(), J() (+21 more)

### Community 71 - "Data Models & Domain (C71)"
Cohesion: 0.08
Nodes (24): APP_DEBUG, APP_ENV, APP_NAME, APP_URL, CACHE_STORE, DB_CONNECTION, DB_DATABASE, DB_HOST (+16 more)

### Community 72 - "Core Module 72"
<<<<<<< HEAD
Cohesion: 0.09
Nodes (21): Aa(), cf(), ef(), fa(), Jc(), Ln(), ma(), pi() (+13 more)

### Community 73 - "HTTP Controllers & Routing (C73)"
Cohesion: 0.21
Nodes (4): PaymentProof, UserDashboard, Livewire\Component, Livewire\WithFileUploads
=======
Cohesion: 0.10
Nodes (25): Aa(), Jc(), Ln(), ma(), pi(), qa(), qc(), Rr() (+17 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 74 - "Data Models & Domain (C74)"
Cohesion: 0.10
Nodes (20): apexcharts, concurrently, laravel-vite-plugin, dependencies, apexcharts, devDependencies, concurrently, laravel-vite-plugin (+12 more)

### Community 75 - "Core Module 75"
Cohesion: 0.10
Nodes (21): _AboutFact, AboutScreen, AppDrawer, BookingSuccessScreen, _ContactInfoCard, _CounterButton, _Field, _FormPage (+13 more)

### Community 76 - "Core Module 76"
<<<<<<< HEAD
Cohesion: 0.08
Nodes (29): actionIsExternal(), canBeConsolidatedWith(), canInvokeAction(), compositionControllerDidBlur(), compositionControllerDidRender(), compositionControllerDidSyncDocumentView(), compositionDidAddAttachment(), compositionDidChangeAttachmentPreviewURL() (+21 more)

### Community 77 - "Core Module 77"
Cohesion: 0.24
Nodes (10): dd(), Jl(), lr(), md(), rd(), uf(), xl(), yr() (+2 more)

### Community 78 - "Core Module 78"
Cohesion: 0.09
Nodes (33): Al(), contains(), count(), determineDataLimits(), Dh(), diff(), endOf(), $f() (+25 more)
=======
Cohesion: 0.07
Nodes (32): actionIsExternal(), canBeConsolidatedWith(), canInvokeAction(), compositionControllerDidBlur(), compositionControllerDidRender(), compositionControllerDidSyncDocumentView(), compositionDidAddAttachment(), compositionDidChangeAttachmentPreviewURL() (+24 more)

### Community 77 - "Core Module 77"
Cohesion: 0.27
Nodes (13): canDecreaseNestingLevel(), canIncreaseNestingLevel(), decreaseNestingLevel(), formatIndent(), formatOutdent(), getLastNestableAttribute(), getListItemAttributes(), getNestableAttributes() (+5 more)

### Community 78 - "Core Module 78"
Cohesion: 0.06
Nodes (57): addElements(), At(), Bs(), buildOrUpdateControllers(), buildOrUpdateElements(), C(), cc(), co() (+49 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 79 - "Core Module 79"
Cohesion: 0.10
Nodes (20): 1. Clone the repository, 1. Navigate to the Flutter folder, 2. Install Flutter Dependencies, 2. Install PHP Dependencies, 3. Install Node Dependencies, 3. Update the API Endpoint, 4. Environment Configuration, 4. Run the App (+12 more)

### Community 80 - "HTTP Controllers & Routing (C80)"
Cohesion: 0.15
Nodes (16): wchar_t, Scale(), Create, Destroy, SetQuitOnClose, Show, UpdateTheme, Win32Window::Win32Window() (+8 more)

### Community 81 - "Core Module 81"
<<<<<<< HEAD
Cohesion: 0.17
Nodes (21): It(), appendAttachmentWithAttributes(), appendBlockForAttributesWithElement(), appendBlockForElement(), appendBlockForTextNode(), appendEmptyBlock(), appendPiece(), appendStringWithAttributes() (+13 more)
=======
Cohesion: 0.22
Nodes (18): appendAttachmentWithAttributes(), appendBlockForAttributesWithElement(), appendBlockForElement(), appendBlockForTextNode(), appendEmptyBlock(), appendPiece(), appendStringWithAttributes(), findBlockElementAncestors() (+10 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 82 - "Core Module 82"
Cohesion: 0.11
Nodes (17): AndroidFlutterLocalNotificationsPlugin, @pragma, dart:io, _downloadAndSaveFile, firebaseMessagingBackgroundHandler, initialize, initializeApp, _localNotifications (+9 more)

### Community 83 - "Core Module 83"
Cohesion: 0.12
Nodes (15): DartProject, HWND, LPARAM, LRESULT, UINT, WPARAM, FlutterWindow, flutter_controller_ (+7 more)

### Community 84 - "Core Module 84"
Cohesion: 0.16
Nodes (5): BookingStatusChart, RecentActivityWidget, RevenueChartWidget, TopRoutesWidget, Filament\Widgets\Widget

<<<<<<< HEAD
### Community 86 - "Data Models & Domain (C86)"
Cohesion: 0.31
Nodes (9): h(), I(), l(), Q(), Re(), ur(), v(), Z() (+1 more)

=======
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288
### Community 87 - "Core Module 87"
Cohesion: 0.12
Nodes (16): require, anhskohbo/no-captcha, dompdf/dompdf, filament/filament, filament/support, intervention/image, kreait/laravel-firebase, laravel/framework (+8 more)

### Community 88 - "Core Module 88"
Cohesion: 0.13
Nodes (15): scripts, dev, post-autoload-dump, post-update-cmd, pre-package-uninstall, test, Composer\\Config::disableProcessTimeout, Illuminate\\Foundation\\ComposerScripts::postAutoloadDump (+7 more)

### Community 89 - "Core Module 89"
Cohesion: 0.14
Nodes (13): addFaq, addQuickFact, addSocialLink, closePanel, removeFaq({{ $fi }}), removeHeroImage({{ (int)$idx }}), removeQuickFact({{ $fi }}), removeSocialLink({{ $li }}) (+5 more)

### Community 90 - "Filament Admin & UI (C90)"
Cohesion: 0.14
Nodes (13): autoload-dev, psr-4, description, keywords, license, minimum-stability, name, prefer-stable (+5 more)

### Community 91 - "Core Module 91"
Cohesion: 0.19
Nodes (12): RegisterPlugins(), OnCreate, HWND, Win32Window, child_content_, GetClientArea, OnCreate, quit_on_close_ (+4 more)

### Community 93 - "Core Module 93"
Cohesion: 0.12
Nodes (15): Cocoa, file_selector_macos, firebase_core, firebase_messaging, RegisterGeneratedPlugins(), MainFlutterWindow, flutter_local_notifications, FlutterMacOS (+7 more)

### Community 94 - "Data Models & Domain (C94)"
Cohesion: 0.15
Nodes (14): Ce(), De(), di(), e(), Ht(), Ie(), Me(), Re() (+6 more)

### Community 95 - "Filament Admin & UI (C95)"
Cohesion: 0.15
Nodes (12): background_color, categories, description, display, icons, name, orientation, short_name (+4 more)

### Community 96 - "Filament Admin & UI (C96)"
Cohesion: 0.24
Nodes (9): wWinMain(), string, wchar_t, CreateAndAttachConsole(), GetCommandLineArguments(), Utf8FromUtf16(), _In_, _In_opt_ (+1 more)

### Community 97 - "Core Module 97"
Cohesion: 0.26
Nodes (7): C(), D(), J(), O(), U(), v(), X()

### Community 98 - "Core Module 98"
Cohesion: 0.18
Nodes (12): Be(), ei(), ii(), le(), ni(), oi(), r(), ri() (+4 more)

### Community 100 - "Filament Admin & UI (C100)"
Cohesion: 0.18
Nodes (10): background_color, description, display, icons, name, orientation, prefer_related_applications, short_name (+2 more)

### Community 101 - "Core Module 101"
Cohesion: 0.36
Nodes (10): HWND, LPARAM, LRESULT, UINT, WPARAM, EnableFullDpiSupportIfAvailable(), GetHandle, GetThisFromHandle (+2 more)

### Community 102 - "Core Module 102"
Cohesion: 0.20
Nodes (9): Flutter & Android Studio Setup Guide, Option A: VS Code (Recommended), Option B: Android Studio, 📋 Prerequisites, 🚀 Step 1: Install the Flutter SDK, 📱 Step 2: Install and Configure Android Studio, 🛠️ Step 3: Run Flutter Doctor, 💻 Step 4: Configure Your IDE (+1 more)

### Community 103 - "Resource"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 104 - "Filament Admin & UI (C104)"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 105 - "Filament Admin & UI (C105)"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 106 - "Filament Admin & UI (C106)"
Cohesion: 0.25
Nodes (8): require-dev, fakerphp/faker, laravel/pail, laravel/pao, laravel/pint, mockery/mockery, nunomaduro/collision, phpunit/phpunit

### Community 107 - "Filament Admin & UI (C107)"
Cohesion: 0.25
Nodes (8): post-root-package-install, setup, composer install, npm install --ignore-scripts, npm run build, @php artisan key:generate, @php artisan migrate --force, @php -r \"file_exists('.env') || copy('.env.example', '.env');\

### Community 108 - "Core Module 108"
Cohesion: 0.33
Nodes (6): build, _checkVersionAndProceed, _goNext, _goToSchedule, _selectTransportOption, MaterialPageRoute

### Community 109 - "Core Module 109"
Cohesion: 0.20
Nodes (11): b(), Dt(), Fe(), g(), He(), i(), ir(), Mt() (+3 more)

### Community 110 - "Core Module 110"
Cohesion: 0.29
Nodes (7): pestphp/pest-plugin, php-http/discovery, config, allow-plugins, optimize-autoloader, preferred-install, sort-packages

### Community 111 - "Core Module 111"
Cohesion: 0.33
Nodes (5): How to Update the Android App (APK), Step 1: Bump the Version Number, Step 2: Build the New APK, Step 3: Copy the New APK to the Web Server, What happens automatically next?

### Community 112 - "Core Module 112"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 113 - "User"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 114 - "Core Module 114"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

<<<<<<< HEAD
### Community 118 - "Database Seeders & Testing (C118)"
Cohesion: 0.09
Nodes (13): BookingController, Request, VoucherService, Request, ScheduleController, Request, VoucherService, VoucherController (+5 more)
=======
### Community 117 - "_each"
Cohesion: 0.11
Nodes (24): ArrowLeft(), ArrowRight(), attachmentManagerDidRequestRemovalOfAttachment(), compositionControllerDidRequestRemovalOfAttachment(), editAttachment(), expandSelectionAroundCommonAttribute(), expandSelectionForEditing(), expandSelectionInDirection() (+16 more)
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 119 - "Core Module 119"
Cohesion: 0.40
Nodes (5): autoload, psr-4, App\\, Database\\Factories\\, Database\\Seeders\\

### Community 120 - "Vehicle"
Cohesion: 0.40
Nodes (4): main, package:flutter_app/main.dart, package:flutter/material.dart, package:flutter_test/flutter_test.dart

### Community 122 - "Core Module 122"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 123 - "Core Module 123"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 124 - "Core Module 124"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 125 - "Core Module 125"
Cohesion: 0.50
Nodes (4): post-create-project-cmd, @php artisan key:generate --ansi, @php artisan migrate --graceful --ansi, @php -r \"file_exists('database/database.sqlite') || touch('database/database.sqlite');\

### Community 126 - "Core Module 126"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 127 - "Core Module 127"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 128 - "Core Module 128"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 129 - "Core Module 129"
Cohesion: 0.50
Nodes (3): Amiga Gracia Flutter App, Getting Started, Railway build

### Community 130 - "Core Module 130"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 131 - "Core Module 131"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 132 - "Core Module 132"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 137 - "Core Module 137"
Cohesion: 0.67
Nodes (3): extra, laravel, dont-discover

### Community 300 - "Filament\Resources\Pages\ViewRecord"
Cohesion: 0.15
Nodes (7): ViewApkUser, ViewBooking, ViewInquiry, ViewPromotionalTicket, ViewTransaction, ViewVoucher, Filament\Resources\Pages\ViewRecord

### Community 301 - "getDataset"
<<<<<<< HEAD
Cohesion: 0.08
Nodes (31): addControllers(), addElements(), addPlugins(), addScales(), beforeUpdate(), buildOrUpdateControllers(), buildOrUpdateElements(), _dataCheck() (+23 more)

### Community 303 - ".nextStep"
Cohesion: 0.16
Nodes (3): Tour, TourDate, Validator
=======
Cohesion: 0.16
Nodes (18): addElements(), buildOrUpdateControllers(), buildOrUpdateElements(), _checkEventBindings(), _dataCheck(), _destroy(), _destroyDatasetMeta(), Ei() (+10 more)

### Community 302 - "VoucherResource"
Cohesion: 0.08
Nodes (3): TransactionResource, VoucherResource, Filament\Infolists\Infolist
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

### Community 307 - "qt"
Cohesion: 0.36
Nodes (8): hs(), Ln(), Nn(), ps(), qt(), Ro(), Se(), wo()

<<<<<<< HEAD
## Knowledge Gaps
- **641 isolated node(s):** `UserSession`, `BookingData`, `UpdateChecker`, `prefs`, `isFirstLaunch` (+636 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **34 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.
=======
### Community 311 - "yn"
Cohesion: 0.33
Nodes (7): ar(), ft(), kn(), sr(), wn(), Ye(), yn()

## Knowledge Gaps
- **594 isolated node(s):** `$schema`, `name`, `type`, `description`, `laravel` (+589 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **35 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

<<<<<<< HEAD
- **Why does `components/chart.js` connect `Data Models & Domain (C1)` to `HTTP Controllers & Routing (C66)`, `Data Models & Domain (C35)`, `Core Module 36`, `Filament Admin & UI (C37)`, `Core Module 6`, `Core Module 10`, `Data Models & Domain (C12)`, `Core Module 77`, `Core Module 14`, `Core Module 78`, `Core Module 47`, `Data Models & Domain (C49)`, `Core Module 17`, `HTTP Controllers & Routing (C19)`, `Data Models & Domain (C52)`, `Core Module 20`, `HTTP Controllers & Routing (C55)`, `Filament Admin & UI (C24)`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `stat/chart.js` connect `Core Module 5` to `Data Models & Domain (C64)`, `Core Module 33`, `Core Module 32`, `Core Module 4`, `Filament Admin & UI (C42)`, `Core Module 43`, `Core Module 10`, `getDataset`, `HTTP Controllers & Routing (C46)`, `Data Models & Domain (C12)`, `Data Models & Domain (C49)`, `Core Module 17`, `HTTP Controllers & Routing (C19)`, `HTTP Controllers & Routing (C55)`, `Data Models & Domain (C26)`, `Core Module 27`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `A()` connect `Core Module 72` to `Core Module 65`, `HTTP Controllers & Routing (C2)`, `Core Module 33`, `Core Module 4`, `Core Module 5`, `Database Seeders & Testing (C70)`, `HTTP Controllers & Routing (C40)`, `Core Module 10`, `Data Models & Domain (C12)`, `Core Module 77`, `Core Module 47`, `Core Module 17`, `Data Models & Domain (C86)`, `Data Models & Domain (C25)`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Are the 4 inferred relationships involving `components/chart.js` (e.g. with `dg()` and `Ms()`) actually correct?**
  _`components/chart.js` has 4 INFERRED edges - model-reasoned connections that need verification._
- **Are the 16 inferred relationships involving `x()` (e.g. with `g()` and `_i()`) actually correct?**
  _`x()` has 16 INFERRED edges - model-reasoned connections that need verification._
- **What connects `UserSession`, `BookingData`, `UpdateChecker` to the rest of the system?**
  _641 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `HTTP Controllers & Routing (C0)` be split into smaller, more focused modules?**
  _Cohesion score 0.006153846153846154 - nodes in this community are weakly interconnected._
=======
- **Why does `A()` connect `Core Module 72` to `Core Module 65`, `HTTP Controllers & Routing (C2)`, `Core Module 33`, `Core Module 4`, `Database Seeders & Testing (C70)`, `HTTP Controllers & Routing (C40)`, `Core Module 10`, `Core Module 43`, `Data Models & Domain (C12)`, `Core Module 17`, `Core Module 56`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `draw()` connect `Data Models & Domain (C12)` to `Data Models & Domain (C1)`, `Core Module 65`, `Core Module 36`, `Core Module 72`, `Core Module 10`, `Core Module 78`, `Core Module 14`, `Data Models & Domain (C52)`, `Core Module 20`, `Core Module 22`, `Filament Admin & UI (C24)`, `Core Module 30`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `Br()` connect `HTTP Controllers & Routing (C40)` to `Core Module 17`, `Core Module 6`, `Data Models & Domain (C94)`, `Data Models & Domain (C1)`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Are the 16 inferred relationships involving `x()` (e.g. with `de()` and `g()`) actually correct?**
  _`x()` has 16 INFERRED edges - model-reasoned connections that need verification._
- **Are the 38 inferred relationships involving `Booking` (e.g. with `.getStaffBookings()` and `.cancel()`) actually correct?**
  _`Booking` has 38 INFERRED edges - model-reasoned connections that need verification._
- **What connects `$schema`, `name`, `type` to the rest of the system?**
  _594 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `HTTP Controllers & Routing (C0)` be split into smaller, more focused modules?**
  _Cohesion score 0.006329113924050633 - nodes in this community are weakly interconnected._
>>>>>>> 8438a3aea74fc104571aabb6df1ba5805453d288
